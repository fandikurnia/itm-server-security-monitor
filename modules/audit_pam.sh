...
# by hand, and both supported families do it - with different
# tools, different template locations and different rules about
# which part of the file is regenerated.
#
#   RHEL / AlmaLinux / Rocky : authselect rewrites the whole file
#                              from a profile
#   Debian / Ubuntu          : pam-auth-update rewrites the block
#                              between its markers from the
#                              profiles in /usr/share/pam-configs
#
# This changes the remediation. Editing the generated region is
# undone the next time the profile is applied, and if the
# attacker touched the template instead of the file, the injected
# line reappears in every host that regenerates.
#
# The Debian case has a twist worth stating in the finding: a
# line appended AFTER the "end of pam-auth-update config" marker
# is outside the managed block and survives regeneration, so it
# persists while looking like it would be cleaned up
# automatically. That is where the injected hooks were found.
pam_is_generated() {
    local file="$1"
    [[ -r "$file" ]] || return 1
    head -20 "$file" 2>/dev/null \
        | grep -qiE 'auto-generated|authselect|pam-auth-update|User changes will be destroyed'
}

# Name the generator, so the note can give commands that exist on
# this host instead of a generic warning.
pam_generator_of() {
    local file="$1"
    [[ -r "$file" ]] || { printf 'unknown'; return 0; }
    if head -20 "$file" 2>/dev/null | grep -qi 'authselect'; then
        printf 'authselect'
    elif head -20 "$file" 2>/dev/null | grep -qi 'pam-auth-update'; then
        printf 'pam-auth-update'
    elif [[ "$ITM_OS_FAMILY" == "rhel" ]] && have_cmd authselect; then
        printf 'authselect'
    elif [[ "$ITM_OS_FAMILY" == "debian" ]] && have_cmd pam-auth-update; then
        printf 'pam-auth-update'
    else
        printf 'unknown'
    fi
...
# One line for the reasons list. The console and the triage
# walker print every reason but only the first line of evidence,
# so anything the operator must see before deciding belongs here.
pam_generated_reason() {
    local file="$1" gen
    pam_is_generated "$file" || return 0
    gen="$(pam_generator_of "$file")"
    printf '
This PAM file is GENERATED by %s: a hand edit can be reverted, and the template must be checked too' \
        "$gen"
}
...
pam_generated_note() {
    ...
    case "$gen" in
        authselect)
            printf '
This file is GENERATED by authselect (RHEL/AlmaLinux/Rocky): the whole file is
rewritten from a profile, so a hand edit is lost at the next apply. Check the
profile too, or the line comes back:
  authselect current
  grep -rn "pam_exec" /etc/authselect/ 2>/dev/null
If the profile is clean, "authselect apply-changes" regenerates the file without
the injected line - which is the cleaner fix on this platform.'
            ;;
        pam-auth-update)
            printf '
This file is GENERATED by pam-auth-update (Debian/Ubuntu): the block between its
markers is rewritten from /usr/share/pam-configs. Check those profiles too:
  grep -rn "pam_exec" /usr/share/pam-configs/ 2>/dev/null
Note that a line placed AFTER the "end of pam-auth-update config" marker sits
OUTSIDE the managed block and survives regeneration - removing it is the only
way it goes away.'
            ;;
        *)
            printf '
This file declares itself auto-generated: editing it directly may be reverted the
next time the generating tool runs. Find the template that produces it before
treating the edit as permanent.'
            ;;
    esac
}
        ...
        while IFS= read -r line; do
            ...
            if (( has_authtok )); then
                add_finding CRITICAL \
                    ...
                    path="$file" \
                    reasons="An ACTIVE pam_exec hook runs ${target:-an unnamed program} during authentication
expose_authtok hands that program the cleartext password on stdin
Every credential authenticated on this host since it was installed must be treated as stolen
No packaged configuration on either supported distribution ships this combination$(pam_generated_reason "$file")" \
...
expose_authtok makes PAM write the cleartext authentication token to this program's stdin." \
            ...
            if [[ "$type_field" == "auth" || "$type_field" == "password" ]]; then
                add_finding CRITICAL \
                    ...
                    path="$file" \
                    reasons="An ACTIVE pam_exec hook runs ${target:-an unnamed program} in the ${type_field} stack
Any program in the auth or password stack observes every authentication attempt
The program is not in PAM_EXEC_ALLOW, so nobody has declared it expected$(pam_generated_reason "$file")" \
                    process="pam stack: $type_field -> ${target:-<no path argument>}" \
                    evidence="Line: $(truncate_text "$line" 200)
...
Any program in the auth or password stack observes authentication attempts." \
            ...
            local session_reasons="A pam_exec hook runs ${target:-an unnamed program} on ${type_field} events
The program is not in PAM_EXEC_ALLOW, so nobody has declared it expected
${why}"
            session_reasons+="$(pam_generated_reason "$file")"
...
The hook used expose_authtok: it received cleartext authentication tokens"
                reasons+="$(pam_generated_reason "$file")"
